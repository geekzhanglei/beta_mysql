...
剧集日历按日期分层缓存：今天的 episode 响应缓存 1 小时，昨天缓存 24 小时，未来日期缓存 12 小时；过期时会立即返回旧缓存并在后台刷新，优先保障小程序首屏速度。
...
API 缓存支持 stale-while-revalidate：缓存过期但仍有旧数据时，普通接口请求会立即返回旧缓存，同时后台刷新并落库，下一次请求自然命中新数据。预热脚本会自动追加 `waitRefresh=1`，让定时任务最多等待 `TMDB_STALE_REFRESH_TIMEOUT` 毫秒（默认 1000）以便按顺序触发刷新。
...
脚本会预热热映、即将上映、趋势、今日播出、昨天到未来 5 天的剧集排期和 episode 日历，并触发部分图片落盘缓存。可通过 `TMDB_WARM_DAYS`、`TMDB_WARM_REGIONS`、`TMDB_WARM_IMAGE_LIMIT`、`TMDB_WARM_EPISODE_LIMIT`、`TMDB_WARM_API_BASE_URL` 调整；如需让预热脚本也立即返回旧缓存，可配置 `TMDB_WARM_WAIT_REFRESH=0`。脚本启动时会清理 7 天前已过期的 `tmdb_api_cache`。
